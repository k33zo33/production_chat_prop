...
if ! command -v "$PYTHON_BIN" >/dev/null 2>&1; then
  echo "[docs-handoff-smoke] missing required binary: $PYTHON_BIN" >&2
  exit 1
fi
...
checks = [
    ...
    ('staged path-filter review reaches the Gemini invocation path' in ai_helper_smoke and
     ...
     'range-plus-path review reaches the Gemini invocation path' in ai_helper_smoke and
     'missing required binary:' in ai_helper_smoke,
     'tool/ai_helper_smoke.sh should keep covering ask/preview-ask, missing-question validation, preview-review, staged path-filter, untracked path-filter, range-plus-path review behavior, and startup binary guards'),
...
set +e
ai_helper_smoke_missing_git_output="$(
  cd "$ROOT_DIR" &&
  GIT_BIN=missing-git "$AI_HELPER_SMOKE_PATH" 2>&1
)"
ai_helper_smoke_missing_git_status=$?
set -e

if [[ "$ai_helper_smoke_missing_git_status" -eq 0 ]]; then
  echo "[docs-handoff-smoke] ai helper smoke missing-git path drifted: expected non-zero status" >&2
  exit 1
fi

if ! grep -Fqx -- "[ai-helper-smoke] missing required binary: missing-git" <<<"$ai_helper_smoke_missing_git_output"; then
  echo "[docs-handoff-smoke] ai helper smoke missing-git output drifted" >&2
  exit 1
fi
...
verify_missing_smoke_common_stub_dir="$(mktemp -d)"
mkdir -p "$verify_missing_smoke_common_stub_dir/tool"
cp "$VERIFY_PATH" "$verify_missing_smoke_common_stub_dir/tool/verify.sh"
chmod +x "$verify_missing_smoke_common_stub_dir/tool/verify.sh"

cat > "$verify_missing_smoke_common_stub_dir/flutter-stub.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
echo "[docs-handoff-smoke] verify missing-smoke-common path should fail before invoking flutter" >&2
exit 99
EOF
chmod +x "$verify_missing_smoke_common_stub_dir/flutter-stub.sh"

set +e
verify_missing_smoke_common_output="$(
  cd "$verify_missing_smoke_common_stub_dir" &&
  FLUTTER_BIN="$verify_missing_smoke_common_stub_dir/flutter-stub.sh" ./tool/verify.sh 2>&1
)"
verify_missing_smoke_common_status=$?
set -e

if [[ "$verify_missing_smoke_common_status" -eq 0 ]]; then
  echo "[docs-handoff-smoke] verify missing-smoke-common path drifted: expected non-zero status" >&2
  rm -rf "$verify_missing_smoke_common_stub_dir"
  exit 1
fi

if ! grep -Fqx -- "[verify] missing required script: $verify_missing_smoke_common_stub_dir/tool/smoke_common.sh" <<<"$verify_missing_smoke_common_output"; then
  echo "[docs-handoff-smoke] verify missing-smoke-common output drifted" >&2
  rm -rf "$verify_missing_smoke_common_stub_dir"
  exit 1
fi
...
compact_required_files=(
  "$compact_missing_file_stub_dir/test/widget_test.dart"
  "$compact_missing_file_stub_dir/test/widget/project_not_found_recovery_test.dart"
  "$compact_missing_file_stub_dir/test/widget/mobile_compact_polish_test.dart"
  "$compact_missing_file_stub_dir/test/widget/playback_empty_state_actions_test.dart"
  "$compact_missing_file_stub_dir/test/widget/scene_status_badge_test.dart"
  "$compact_missing_file_stub_dir/test/widget/focus_preview_autofollow_test.dart"
  "$compact_missing_file_stub_dir/test/widget/focus_preview_chrome_test.dart"
  "$compact_missing_file_stub_dir/test/widget/focus_preview_short_height_test.dart"
  "$compact_missing_file_stub_dir/test/widget/short_height_entry_states_test.dart"
)

for missing_file in "${compact_required_files[@]}"; do
  rm -rf "$compact_missing_file_stub_dir/test"
  mkdir -p "$compact_missing_file_stub_dir/test/widget"

  for present_file in "${compact_required_files[@]}"; do
    if [[ "$present_file" == "$missing_file" ]]; then
      continue
    fi
    cp "$ROOT_DIR/${present_file#$compact_missing_file_stub_dir/}" "$present_file"
  done
  ...
  if [[ "$compact_missing_file_status" -eq 0 ]]; then
    echo "[docs-handoff-smoke] compact smoke missing-file path drifted: expected non-zero status for $missing_file" >&2
    rm -rf "$compact_missing_file_stub_dir"
    exit 1
  fi

  expected_missing_line="[compact-smoke] missing expected test file: ${missing_file#$compact_missing_file_stub_dir/}"
  if ! grep -Fqx -- "$expected_missing_line" <<<"$compact_missing_file_output"; then
    echo "[docs-handoff-smoke] compact smoke missing-file output drifted for $missing_file" >&2
    rm -rf "$compact_missing_file_stub_dir"
    exit 1
  fi
done
...
set +e
demo_missing_flutter_output="$(
  cd "$ROOT_DIR" &&
  FLUTTER_BIN=missing-flutter SMOKE_SKIP_VERSION=1 SMOKE_SKIP_ANALYZE=1 "$DEMO_SMOKE_PATH" 2>&1
)"
demo_missing_flutter_status=$?
set -e

if [[ "$demo_missing_flutter_status" -eq 0 ]]; then
  echo "[docs-handoff-smoke] demo smoke missing-flutter path drifted: expected non-zero status" >&2
  exit 1
fi

if ! grep -Fqx -- "[demo-smoke] missing required binary: missing-flutter" <<<"$demo_missing_flutter_output"; then
  echo "[docs-handoff-smoke] demo smoke missing-flutter output drifted" >&2
  exit 1
fi
...
release_missing_smoke_common_stub_dir="$(mktemp -d)"
mkdir -p "$release_missing_smoke_common_stub_dir/tool"
cp "$RELEASE_SMOKE_PATH" "$release_missing_smoke_common_stub_dir/tool/release_smoke.sh"
chmod +x "$release_missing_smoke_common_stub_dir/tool/release_smoke.sh"

cat > "$release_missing_smoke_common_stub_dir/flutter-stub.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
echo "[docs-handoff-smoke] release smoke missing-smoke-common path should fail before invoking flutter" >&2
exit 99
EOF
chmod +x "$release_missing_smoke_common_stub_dir/flutter-stub.sh"

set +e
release_missing_smoke_common_output="$(
  cd "$release_missing_smoke_common_stub_dir" &&
  FLUTTER_BIN="$release_missing_smoke_common_stub_dir/flutter-stub.sh" ./tool/release_smoke.sh 2>&1
)"
release_missing_smoke_common_status=$?
set -e

if [[ "$release_missing_smoke_common_status" -eq 0 ]]; then
  echo "[docs-handoff-smoke] release smoke missing-smoke-common path drifted: expected non-zero status" >&2
  rm -rf "$release_missing_smoke_common_stub_dir"
  exit 1
fi

if ! grep -Fqx -- "[release-smoke] missing required script: $release_missing_smoke_common_stub_dir/tool/smoke_common.sh" <<<"$release_missing_smoke_common_output"; then
  echo "[docs-handoff-smoke] release smoke missing-smoke-common output drifted" >&2
  rm -rf "$release_missing_smoke_common_stub_dir"
  exit 1
fi
...
set +e
release_missing_flutter_output="$(
  cd "$ROOT_DIR" &&
  FLUTTER_BIN=missing-flutter SMOKE_SKIP_VERSION=1 SMOKE_SKIP_ANALYZE=1 "$RELEASE_SMOKE_PATH" 2>&1
)"
release_missing_flutter_status=$?
set -e

if [[ "$release_missing_flutter_status" -eq 0 ]]; then
  echo "[docs-handoff-smoke] release smoke missing-flutter path drifted: expected non-zero status" >&2
  exit 1
fi

if ! grep -Fqx -- "[release-smoke] missing required binary: missing-flutter" <<<"$release_missing_flutter_output"; then
  echo "[docs-handoff-smoke] release smoke missing-flutter output drifted" >&2
  exit 1
fi
