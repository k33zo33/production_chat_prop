...
result="$(run_and_capture "$TMP_DIR/tool/ai_helper.sh" ask "Summarize helper fallback risks.")"
ask_status="$(printf '%s\n' "$result" | head -n1)"
ask_output="$(printf '%s\n' "$result" | tail -n +2)"
assert_status 1 "$ask_status" "ask with question"
assert_output_contains "===== GEMINI CLI ANALYSIS =====" "$ask_output" "ask with question"
assert_output_contains "[ai-helper] Gemini helper unavailable." "$ask_output" "ask with question"

result="$(run_and_capture "$TMP_DIR/tool/ai_helper.sh" preview-ask)"
preview_ask_missing_status="$(printf '%s\n' "$result" | head -n1)"
preview_ask_missing_output="$(printf '%s\n' "$result" | tail -n +2)"
assert_status 1 "$preview_ask_missing_status" "preview-ask missing question"
assert_output_contains "preview-ask mode requires a question" "$preview_ask_missing_output" "preview-ask missing question"

result="$(run_and_capture "$TMP_DIR/tool/ai_helper.sh" ask)"
ask_missing_status="$(printf '%s\n' "$result" | head -n1)"
ask_missing_output="$(printf '%s\n' "$result" | tail -n +2)"
assert_status 1 "$ask_missing_status" "ask missing question"
assert_output_contains "ask mode requires a question" "$ask_missing_output" "ask missing question"
...
echo "[ai-helper-smoke] ask with a real question reaches the Gemini invocation path"
echo "[ai-helper-smoke] preview-ask and ask reject missing questions clearly"
