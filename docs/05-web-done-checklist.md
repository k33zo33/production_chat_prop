...
- `./tool/compact_smoke.sh` passed for targeted compact/export regressions, including narrow delete confirmations, long project-name/header clamping, short-landscape compact navigation/focus-preview flows, short-height empty/recovery entry shells, project-list search/filter/sort controls, portfolio-readiness CTA flows, stale-link recovery paths, ultra-compact editor/playback footer stacking, and focus-preview chrome stacking at larger text
