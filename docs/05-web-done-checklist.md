...
- [x] GitHub Actions runs `./tool/docs_handoff_smoke.sh` as a standalone docs/checklist/workflow preflight before the full `beta_handoff` gate
...
- GitHub Actions now also runs `./tool/docs_handoff_smoke.sh` as a separate `docs_handoff_smoke` job, and the main `beta_handoff` job explicitly waits for it so docs/checklist/workflow drift gets caught before the heavier Flutter gate
